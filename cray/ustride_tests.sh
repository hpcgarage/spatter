...
outfilebase=$SPATTER_BUILD/ustride_tests_$PROC
...
echo -n "# Compiler: " >> $outfile
cat $SPATTER_BUILD/COMPILER >> $outfile 
echo >> $outfile
echo -n "# Optimizations: " >> $outfile
cat $SPATTER_BUILD/OPTIMIZATIONS >> $outfile 
echo >> $outfile 
echo >> $outfile 
...
do
    ...
    do
        ...
        for i in `seq 1 $run`;
        do
            $SPATTER_BUILD/spatter-$PROC -R $loop -q --ustride=$stride,$delta --nph -l $len  | grep GATHER | cut -d' ' -f9 >> tmp$stride.txt;
        done
