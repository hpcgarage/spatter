...
int cpu_amg_test() {
  ...
  int ret = asprintf(&command,
      "../src/spatter-driver -f ../../standard-suite/app-traces/amg.json");
  if (ret == -1 || system(command) != EXIT_SUCCESS) {
    std::cerr << "Test failure on " << command << std::endl;
    return EXIT_FAILURE;
  }
...
int cpu_lulesh_test() {
  ...
  int ret = asprintf(&command,
      "../src/spatter-driver -f ../../standard-suite/app-traces/lulesh.json");
  if (ret == -1 || system(command) != EXIT_SUCCESS) {
    std::cerr << "Test failure on " << command << std::endl;
    return EXIT_FAILURE;
  }
...
int cpu_nekbone_test() {
  ...
  int ret = asprintf(&command,
      "../src/spatter-driver -f ../../standard-suite/app-traces/nekbone.json");
  if (ret == -1 || system(command) != EXIT_SUCCESS) {
    std::cerr << "Test failure on " << command << std::endl;
    return EXIT_FAILURE;
  }
...
int cpu_pennant_test() {
  ...
  int ret = asprintf(&command,
      "../src/spatter-driver -f ../../standard-suite/app-traces/pennant.json");
  if (ret == -1 || system(command) != EXIT_SUCCESS) {
    std::cerr << "Test failure on " << command << std::endl;
    return EXIT_FAILURE;
  }
