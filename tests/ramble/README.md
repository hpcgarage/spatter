...
The python file `example_ramble_test.py` automatically creates a workspace and runs both experiments. You can run it with the following command:
```
To be updated!
```
...
Run the following commands to setup a new experiment for spatter. Copy `application.py` into `spatter/applications/spatter`
```
