...
int laplacian_test() {
    ...
    for (int i = 0; i < 10; i++) {
        const char *command = sprintf("./spatter -pLAPLACIAN:%d:%d:%d", dimension, pseudo_order, problem_size);
...
int laplacian_test_delta() {
    for (int delta = 1; delta < 100; delta*=2) {
        const char *command = sprintf("./spatter -pLAPLACIAN:2:2:100 -d%d", delta);
        if (system(command) == EXIT_FAILURE) {
            printf("Test failure on %s", command);
            return EXIT_FAILURE;
        }
