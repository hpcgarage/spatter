...
int main(int argc, char **argv) {
  ...
  // Unlike parse_run_config_suite_gpu.cc, which leaves the backend at its
  // default, this passes -b so the Tenstorrent Configuration is actually
  // constructed. The CI job that runs it holds a card.
  ret = asprintf(&argv_[3], "-btenstorrent");
  if (ret == -1)
    return EXIT_FAILURE;
