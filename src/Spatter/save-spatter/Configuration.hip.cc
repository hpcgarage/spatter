...
Configuration<Spatter::CUDA>::~Configuration() {
  checkCudaErrors(hipFree(dev_pattern));
  checkCudaErrors(hipFree(dev_pattern_gather));
  checkCudaErrors(hipFree(dev_pattern_scatter));

  if (dev_sparse) {
    checkCudaErrors(hipFree(dev_sparse));
    dev_sparse = nullptr;
  }

  if (dev_sparse_gather) {
    checkCudaErrors(hipFree(dev_sparse_gather));
    dev_sparse_gather = nullptr;
  }

  if (dev_sparse_scatter) {
    checkCudaErrors(hipFree(dev_sparse_scatter));
    dev_sparse_scatter = nullptr;
  }

  if (dev_dense) {
    checkCudaErrors(hipFree(dev_dense));
    dev_dense = nullptr;
  }
}
...
void Configuration<Spatter::CUDA>::setup() {
  ...
  checkCudaErrors(hipMemcpy(dev_pattern, pattern.data(),
      sizeof(size_t) * pattern.size(), hipMemcpyHostToDevice));
  checkCudaErrors(hipMemcpy(dev_pattern_gather, pattern_gather.data(),
      sizeof(size_t) * pattern_gather.size(), hipMemcpyHostToDevice));
  checkCudaErrors(hipMemcpy(dev_pattern_scatter, pattern_scatter.data(),
