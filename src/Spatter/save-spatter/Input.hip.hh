...
#ifdef USE_CUDA
  if (backend.compare("cuda") == 0) {
    checkCudaErrors(hipMalloc((void **)&cl.dev_sparse,
        sizeof(double) * cl.sparse.size()));
    checkCudaErrors(hipMalloc((void **)&cl.dev_sparse_gather,
        sizeof(double) * cl.sparse_gather.size()));
    checkCudaErrors(hipMalloc((void **)&cl.dev_sparse_scatter,
        sizeof(double) * cl.sparse_scatter.size()));
    checkCudaErrors(hipMalloc((void **)&cl.dev_dense,
        sizeof(double) * cl.dense.size()));

    checkCudaErrors(hipMemcpy(cl.dev_sparse, cl.sparse.data(),
        sizeof(double) * cl.sparse.size(), hipMemcpyHostToDevice));
    checkCudaErrors(hipMemcpy(cl.dev_sparse_gather, cl.sparse_gather.data(),
        sizeof(double) * cl.sparse_gather.size(), hipMemcpyHostToDevice));
    checkCudaErrors(hipMemcpy(cl.dev_sparse_scatter, cl.sparse_scatter.data(),
        sizeof(double) * cl.sparse_scatter.size(), hipMemcpyHostToDevice));
    checkCudaErrors(hipMemcpy(cl.dev_dense, cl.dense.data(),
