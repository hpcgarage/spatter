...
std::unique_ptr<Spatter::ConfigurationBase> JSONParser::operator[](
  ...
  if (backend_.compare("serial") == 0)
    c = std::make_unique<Spatter::Configuration<Spatter::Serial>>(index,
        ...
        sparse_scatter, sparse_scatter_size, dense, dense_size, delta,
        delta_gather, delta_scatter, data_[index]["seed"], data_[index]["wrap"],
        data_[index]["count"], data_[index]["nruns"], aggregate_, compress_,
        verbosity_);
#ifdef USE_OPENMP
    ...
    c = std::make_unique<Spatter::Configuration<Spatter::CUDA>>(index,
        ...
        pattern_scatter, sparse, sparse_size, sparse_gather, sparse_gather_size,
        sparse_scatter, sparse_scatter_size, dense, dense_size, delta,
        delta_gather, delta_scatter, data_[index]["seed"],
